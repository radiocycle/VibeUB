...
def ensure_api_credentials(config_manager) -> None:
    ...
    if not sys.stdin.isatty():
        raise RuntimeError("API credentials are missing in config.json and interactive input is unavailable.")
    ...
    if config.api_id or config.api_hash:
        print("Stored credentials are invalid. Enter a real api_id/api_hash pair.\n")
